...
void MainWindow::dfs(int i,int j, QColor color){
    if(i<0 || j<0 || i>size_of_the_frame || j>size_of_the_frame || sourceImage.pixelColor(i,j)!=QColor(0,255,0)){

    }
    else {
        point(sourceImage,i,j,3,color.red(),color.green(),color.blue());
        delay(DELAYTIME);
        dfs(i+pixelsize,j,color);
        dfs(i,j+pixelsize,color);
        dfs(i-pixelsize,j,color);
        dfs(i,j-pixelsize,color);
    }
}
void MainWindow::on_pushButton_2_clicked()
{
    ...
    int y = pixelsize/2;
    int res=0;
    for(int i=x;i<size_of_the_frame;i=i+pixelsize){
        for(int j=y;j<size_of_the_frame;j=j+pixelsize){
            if(sourceImage.pixelColor(i,j)==QColor(0,255,0)){
                res++;
                dfs(i,j,QColor(rand()%128+128,rand()%128+128,rand()%128+128));
            }
        }
    }
    ...
    cout<<res<<endl;
        ...
        ui->label_2->setText( QString::number(res)+" Islands are there");
